...
from resources.scripts.case_study import extract_logic, load_logic, update_ingest_type_logic
...
def case_study():
    ...
    for item in config.get("ingestion", []):
        ...
        start_task >> extract(item) >> load(item) >> update_ingest_type(item) >> wait_el_task
